...
#if __has_include(<com/meta/IPMI/UnifiedSEL/event.hpp>)
#include <com/meta/IPMI/UnifiedSEL/event.hpp>
#define PLDM_HAVE_META_UNIFIED_SEL_EVENT 1
#endif
    ...
    pldm_tid_t tid, const uint8_t* eventData, size_t eventDataSize) const
{
    ...
    std::string message =
        "SEL Entry: FRU: " + slotNumber + ", Record: " + errorLog;
...
#ifdef PLDM_HAVE_META_UNIFIED_SEL_EVENT
    {
...
#else
    lg2::info(
        "META UnifiedSEL generated event interface unavailable; journal-only OEM event logged for source={SOURCE}",
        "SOURCE", source);
#endif
