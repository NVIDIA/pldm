...
TEST(PlatformHandlerWrapper, pdrRepositoryChangeEventAdditionalCoverage)
{
    ...
    pldm::HostPDRHandler hostPDRHandler(
        0, 8, event, hostRepo, "./event_jsons/good", entityTree, bmcEntityTree,
        instanceIdDb, &reqHandler);
    Handler handler(&mockedUtils, "./pdr_jsons/state_sensor/good", platformRepo,
                    &hostPDRHandler, nullptr, nullptr, nullptr, event);
    ...
    uint8_t platformEventStatus = PLDM_EVENT_NO_LOGGING;
    std::vector<uint8_t> request(sizeof(pldm_msg_hdr) + eventDataLength, 0);
    auto* msg = asMsg(request);
    std::copy(eventData.begin(), eventData.end(), msg->payload);
    EXPECT_EQ(handler.pldmPDRRepositoryChgEvent(
                  msg, eventDataLength,
                  PLDM_PLATFORM_EVENT_MESSAGE_FORMAT_VERSION, 9, 0,
                  platformEventStatus),
              PLDM_SUCCESS);
    ...
    auto invalidEntries = eventData;
    ASSERT_GT(invalidEntries.size(), 3u);
    invalidEntries[3] = 2;
    std::vector<uint8_t> invalidRequest(
        sizeof(pldm_msg_hdr) + invalidEntries.size(), 0);
    auto* invalidMsg = asMsg(invalidRequest);
    std::copy(invalidEntries.begin(), invalidEntries.end(),
              invalidMsg->payload);
    EXPECT_EQ(handler.pldmPDRRepositoryChgEvent(
                  invalidMsg, invalidEntries.size(),
                  PLDM_PLATFORM_EVENT_MESSAGE_FORMAT_VERSION, 9, 0,
                  platformEventStatus),
              PLDM_ERROR_INVALID_DATA);

    const std::array<uint8_t, 3> malformedInner{FORMAT_IS_PDR_HANDLES, 1,
                                                PLDM_RECORDS_ADDED};
    std::vector<uint8_t> malformedRequest(
        sizeof(pldm_msg_hdr) + malformedInner.size(), 0);
    auto* malformedMsg = asMsg(malformedRequest);
    std::copy(malformedInner.begin(), malformedInner.end(),
              malformedMsg->payload);
    EXPECT_NE(handler.pldmPDRRepositoryChgEvent(
                  malformedMsg, malformedInner.size(),
                  PLDM_PLATFORM_EVENT_MESSAGE_FORMAT_VERSION, 9, 0,
                  platformEventStatus),
              PLDM_SUCCESS);
