...
class RequestIntfTest : public testing::Test
{
  protected:
    ...
    pldm::mctp_socket::Handler* handler = nullptr;
    std::vector<uint8_t> requestMsg;
};

TEST_F(RequestIntfTest, 0Retries100msTimeout)
{
    MockRequest request(fd, eid, event, handler, std::move(requestMsg), 0,
...
{
    MockRequest request(fd, eid, event, handler, std::move(requestMsg), 2,
                        milliseconds(100), false);
...
{
    MockRequest request(fd, eid, event, handler, std::move(requestMsg), 9,
                        milliseconds(100), false);
