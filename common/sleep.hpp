...
struct Sleep
{
    ...
    bool await_suspend(std::coroutine_handle<> handle) noexcept
    {
        ...
        {
            rc = PLDM_ERROR; // Failure scenario handling
        ...
        if (sd_event_add_time(
            ...
            0)
        {
            rc = PLDM_ERROR; // Handle errors from adding the timer
            return false;    // Don't suspend on failure
        }
     ...
     */
    pldm_completion_codes await_resume() const noexcept
    {
        return rc;
    }
