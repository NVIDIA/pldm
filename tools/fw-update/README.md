...
specification v1.3.0 - Section 7.
...
Records. Each such record can have the following properties:
    ...
    - ReferenceManifestLength: Supported for package versions 1.3 and above,
...
Supported for package versions greater than 1.0 with the following limitations:

    - DownstreamDeviceIDRecordCount: Supported, automatically set to 0 (no downstream devices)
    - DownstreamDeviceIDRecords: Not supported - only packages with 0 downstream device
...
Information records. Each such record can have the following properties:
    ...
    - ComponentOpaqueDataLength: Supported for package versions 1.2 and above.
      Currently always set to 0 (ComponentOpaqueData not yet fully supported).
